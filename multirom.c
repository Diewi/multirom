...
int multirom(const char *rom_to_boot)
{
    ...
    if(to_boot)
    {
        ...
        if (s.is_second_boot != 0)
        {
            ...
            {
                ERROR("Could not flash the primary boot image back.\n");
                MR_NO_KEXEC_ABORT;
            }
        }
        else
        {
            if ((to_boot->type != ROM_DEFAULT) && to_boot->has_bootimg)
            {
                ...
                {
                    ERROR("Could not flash the secondary boot image.\n");
                    MR_NO_KEXEC_ABORT;
                }
...
int multirom_fill_kexec_android(struct multirom_status *s, struct multirom_rom *rom, struct kexec *kexec)
{
    ...
    res = remove_dtb_mounts(fdt_path, s);
    if (res < 0)
    {
        ERROR("Error while analyzing/modifying FDT.\n");
        return -1;
    }

        ...
        const char *path_bootimg_mod, struct multirom_status *status)
{
    int res = 0;
    struct bootimg img;
    const char *path_dtb_img ="/dtb.img";
    ...
    res = libbootimg_dump_dtb(&img, path_dtb_img);
    if (res != 0)
    {
    ...
    res = remove_dtb_mounts(path_dtb_img, status);
    if (res < 0)
    {
        INFO("%s: Could not dump bootimage.\n", __func__);
        return res;
    }
    else if (status->dtb_removed_partitions == DTB_PARTITION_NONE)
    {
        return 0;
    }

    if (status->dtb_removed_partitions != DTB_PARTITION_NONE)
    {
        res = libbootimg_load_dtb(&img, path_dtb_img);
        if (res != 0)
        {
            INFO("%s: Could not dump bootimage.\n", __func__);
            return res;
